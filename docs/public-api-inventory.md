...
Expansion tranche controls (`DashboardGrid`, `QuickOpenOverlay`, `BulletChart`, `ResizablePaneGroup`, `SideNavRail`, `TokenEditor`, `HealthBoard`, `JumpList`, `AutocompleteInput`, `BoxPlot`) are implemented with dedicated tests; theme extension mapping is wired for `BulletChart`, `DashboardGrid`, `HealthBoard`, `ResizablePaneGroup`, `SideNavRail`, `TokenEditor`, `JumpList`, `AutocompleteInput`, and `BoxPlot` (`TeaThemeControlExtensions.DashboardMetrics.cs`, `TeaThemeControlExtensions.FormsAndShell.cs`, `TeaThemeControlExtensions.Navigation.cs`, `TeaThemeControlExtensions.NavigationOverlay.cs`, `TeaThemeControlExtensions.PlanningAndBoards.cs`, `TeaThemeControlExtensions.Plotting.cs`), while `QuickOpenOverlay` remains pending `ApplyTheme`/`ApplyThemeDefaults` parity wiring.
