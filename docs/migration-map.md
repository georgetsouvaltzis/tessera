...
- `Stack` / `Split` / `Panel` / `Dock` / `Overlay` / `Center` / `Slot` static helper DSLs -> removed; replace with `WindowLayout`, `RowLayout`, `ColumnLayout`, `PanelLayout`, `CenterLayout`, `LayoutSlot`

## Notes

- `TextBlockComponent`, `ButtonComponent`, `TextInputComponent`, `TextAreaComponent`, `StatusBarComponent`, `DropdownComponent`, `ComboboxComponent`, and `ListComponent<T>` are removed, not merely hidden.
