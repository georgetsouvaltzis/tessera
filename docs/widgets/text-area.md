...
Use `TextArea` when this interaction is the best match for your screen workflow.

## When to use

- You need a `TextArea`-style interaction inside the inputs & forms lane.
...
- Do not choose `TextArea` by name only; validate it against the target workflow.
...
| `DisabledValueTextStyle` | `TesseraStyle` |
