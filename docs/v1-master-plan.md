...
- item `3`: **In progress** (consumer ergonomics helpers now include dialog/result, table row mutation, header row composition, theme override bundles, auto-rescheduling periodic effects, and selection API normalization/alias coverage; broader API-surface consistency still open, including final post-V1 `Current*` -> `Selected*` primary-name cleanup policy)
