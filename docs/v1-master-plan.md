...
- item `3`: **In progress** (ergonomics and naming convergence advanced via `4d4a391` + `e731edc`; residual external-consumer gaps remain explicit: `Table` selection APIs, `ListView<T>` programmatic selection setter, and no first-party `ThemeScope`-style aggregate theme fan-out helper)
