...
public sealed class TreeMapChart : Control
{
    ...
    private void LegendBand(Canvas canvas, int start, ref int cursor, int y, int width, char glyph, string label, TeaStyle style)
    {
    ...
    private (char Glyph, TeaStyle Style) ResolveBand(double value, double min, double max)
    {
    ...
    private TeaStyle ResolveStateStyle(int leafIndex)
    {
    ...
    private TeaStyle ResolveStyle(TeaStyle style) => IsDisabled ? style.Merge(DisabledNodeStyle) : style;
    private TeaStyle ResolveBorderStyle() => ResolveStyle(IsFocused ? BorderStyleText.Merge(FocusedBorderStyleText) : BorderStyleText);
    ...
    private static void DrawFill(Canvas canvas, Rect bounds, char glyph, TeaStyle style)
    {
        if (style.IsEmpty)
        {
    ...
    private static void WriteText(Canvas canvas, int x, int y, string text, TeaStyle style, int width) => canvas.WriteText(x, y, Style(text ?? string.Empty, style), width);
    private static string Style(string text, TeaStyle style) => style.IsEmpty ? text : style.Render(text);
