...
public sealed partial class ActivityFeed
{
    ...
    private TeaStyle ResolveKindStyle(ActivityFeedItemKind kind)
    {
        return kind switch
        {
    ...
    private static string ApplyStyle(string text, TeaStyle style) => style.IsEmpty ? text : style.Render(text);
}
