...
public sealed partial class DataForm<TModel>
{
    ...
    private bool TryResolveStatusLine(out string text, out TeaStyle style)
    {
        if (IsDisabled)
        {
    ...
    private TeaStyle ResolvePrefixStyle(int index, DataFormField<TModel> field)
    {
    ...
    private TeaStyle ResolveValueSegmentStyle(int index, DataFormField<TModel> field, bool isPlaceholder)
    {
        var style = TeaStyle.Empty;
        if (field.IsReadOnly || !field.CanWrite)
        {
