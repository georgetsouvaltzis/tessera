...
public sealed class InspectorPanel : Control
{
    ...
    {
        var selectedStyle = selected ? ResolveSelectedStyle() : TesseraStyle.Empty;
        ...
        if (row.Kind == InspectorRowKind.Field)
        {
            ...
            Write(canvas, content.X, y, string.Concat("  ", key, " : ", value), TesseraStyle.Empty, content.Width);
            return;
        }
        ...
        Write(canvas, content.X, y, text, TesseraStyle.Empty, content.Width);
    }
    ...
    private TesseraStyle ResolveSelectedStyle()
    {
