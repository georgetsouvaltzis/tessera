...
public sealed class TaskRunnerPanel : Control
{
    ...
    public TaskRunItem? SelectedItem =>
    ...
    public override void Render(Canvas canvas, Rect rect)
    {
        ...
        if (_items.Count == 0)
        {
            ...
            WriteStyledText(canvas, content.X, content.Y, EmptyText, style, content.Width);
            return;
        }
    ...
    internal override LayoutMeasurement Measure(in Rect availableBounds)
    {
        ...
        {
            var markerWidth = Math.Max(1,
                ControlTextLayout.MeasureDisplayWidth(ResolveStatusMarker(_items[index].Status)));
            var bodyWidth = ControlTextLayout.MeasureDisplayWidth(BuildRowBody(_items[index]));
            width = Math.Max(width,
                markerWidth + 1 + bodyWidth + Padding.Horizontal + (Border == BorderStyle.None ? 0 : 2));
        }
    ...
    private void RaiseSelectionChangedIfNeeded(int previousIndex, TaskRunItem? previousItem)
    {
        ...
        SelectionChanged?.Invoke(this,
            new TaskRunnerSelectionChangedEventArgs(previousIndex, SelectedIndex, previousItem, SelectedItem));
    }
