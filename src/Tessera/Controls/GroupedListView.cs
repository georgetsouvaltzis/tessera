...
public sealed class GroupedListView<TGroup, TItem> : Control
{
    ...
    public TesseraStyle GroupHeaderStyle { get; set; } = TesseraStyle.Empty;
    ...
    public int? SelectedGroupIndex => TryGetRow(SelectedRowIndex, out var row) ? row.GroupIndex : null;

    /// <summary>Selected item index within group when item row is selected.</summary>
    public int? SelectedItemIndex => TryGetRow(SelectedRowIndex, out var row) && !row.IsHeader ? row.ItemIndex : null;

    /// <summary>Selected item when item row is selected.</summary>
    public TItem? SelectedItem => TryGetRow(SelectedRowIndex, out var row) && !row.IsHeader
        ? _groups[row.GroupIndex].Items[row.ItemIndex]
        : default;
    ...
    {
        if (IsDisabled || IsReadOnly || !IsFocused || _rows.Count == 0 || message is not KeyPressed key)
        {
            return false;
        }
        ...
        {
            return SetSelectedRowIndex(SelectedRowIndex + Math.Max(1, _viewportHeight));
        }
        ...
        if (key.Is(Key.End))
        {
            return SetSelectedRowIndex(_rows.Count - 1);
        }

        if (key.Is(Key.Left))
        {
            return SetSelectedGroupCollapse(true);
        }

        if (key.Is(Key.Right))
        {
            return SetSelectedGroupCollapse(false);
        }

        if (key.Is(Key.Enter) || key.IsCharacter(' '))
        {
            if (TryGetRow(SelectedRowIndex, out var row) && row.IsHeader)
            {
                return ToggleGroup(row.GroupIndex);
            }
    ...
    public override bool Handle(Message message, Rect bounds)
    {
        ...
        if (pointer.Kind == PointerEventKind.Wheel)
        {
            ...
            {
                return SetSelectedRowIndex(SelectedRowIndex + 1) || changed;
            }
    ...
    public override void Render(Canvas canvas, Rect rect)
    {
        ...
        if (_rows.Count == 0)
        {
            canvas.WriteText(content.X, content.Y,
                ApplyStyle(EmptyText, ResolveRowStyle(TesseraStyle.Empty, false, false)), content.Width);
            return;
        }
        ...
        for (var row = 0; row < rows; row++)
        {
            ...
            var text = entry.IsHeader
                ? BuildHeaderText(entry, selected, hovered)
                : BuildItemText(entry, selected, hovered);
            canvas.WriteText(content.X, content.Y + row,
                ApplyStyle(text, ResolveRowStyle(baseStyle, selected, hovered)), content.Width);
        }
    }

    internal override LayoutMeasurement Measure(in Rect availableBounds)
    {
        var width = Math.Max(12, ControlTextLayout.MeasureDisplayWidth(MeasureTitle()) + 4) + Padding.Horizontal +
    ...
    {
        if (!TryGetRow(SelectedRowIndex, out var row))
        {
            return false;
        }

        var groupIndex = row.GroupIndex;
        if (groupIndex < 0 || groupIndex >= _groups.Count || _groups[groupIndex].IsCollapsed == collapse)
        {
            return false;
        }
    ...
    private void RebuildRows(SelectionSnapshot previous)
    {
        ...
        {
            _rows.Add(new RowEntry(g, -1, true));
            if (_groups[g].IsCollapsed)
            {
                continue;
            }

            for (var i = 0; i < _groups[g].Items.Count; i++)
            {
                _rows.Add(new RowEntry(g, i, false));
            }
        }
        ...
        SelectedRowIndex = ResolveRestoredRow(previous);
    ...
    private int ResolveRestoredRow(SelectionSnapshot previous)
    {
        ...
        return SelectedRowIndex >= 0 ? Math.Clamp(SelectedRowIndex, 0, _rows.Count - 1) : 0;
    }
    ...
    private int FindRowIndex(int groupIndex, int itemIndex)
    {
        ...
        {
            var row = _rows[i];
            if (row.GroupIndex == groupIndex && row.ItemIndex == itemIndex)
            {
                return i;
            }
        }
    ...
    private SelectionSnapshot CaptureSelection()
    {
        if (!TryGetRow(SelectedRowIndex, out var row))
        {
            return new SelectionSnapshot(-1, null, null, default);
        }

        var item = row.IsHeader ? default : _groups[row.GroupIndex].Items[row.ItemIndex];
        return new SelectionSnapshot(SelectedRowIndex, row.GroupIndex, row.IsHeader ? null : row.ItemIndex, item);
    }
    ...
    private TesseraStyle ResolveRowStyle(TesseraStyle baseStyle, bool selected, bool hovered)
    {
        var style = baseStyle;
        if (hovered)
        {
    ...
    private static string DefaultGroupText(TGroup value)
    {
        return value?.ToString() ?? string.Empty;
    }

    private static string DefaultItemText(TItem value)
    {
        return value?.ToString() ?? string.Empty;
    }
