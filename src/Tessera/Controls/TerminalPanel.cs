...
public sealed class TerminalPanel : Control
{
    ...
    /// </summary>
    public TesseraStyle StandardOutputStyle { get; set; } = TesseraStyle.Empty;

    /// <summary>
    ///     Gets or sets base style for standard error rows.
    /// </summary>
    public TesseraStyle StandardErrorStyle { get; set; } = TesseraStyle.Empty;
    ...
    public TerminalPanelLine? SelectedLine =>
        SelectedIndex >= 0 && SelectedIndex < _lines.Count
    ...
    public void Append(string text, TerminalPanelChannel channel = TerminalPanelChannel.StandardOutput,
        string? marker = null)
    {
    ...
    internal override LayoutMeasurement Measure(in Rect availableBounds)
    {
        ...
        for (var index = 0; index < _lines.Count; index++)
        {
            width = Math.Max(width,
                ControlTextLayout.MeasureDisplayWidth(FormatLine(index, _lines[index])) + Padding.Horizontal);
        }
    ...
    {
        if (SelectedIndex < 0 || _lines.Count == 0)
        {
            return;
        }
    ...
    private void TrimToMaxLines()
    {
        ...
        if (SelectedIndex >= 0)
        {
            SelectedIndex = Math.Max(-1, SelectedIndex - overflow);
        }

        if (_hoveredIndex >= 0)
        {
            _hoveredIndex = Math.Max(-1, _hoveredIndex - overflow);
        }
    ...
    private TesseraStyle ResolveChannelStyle(TerminalPanelChannel channel)
    {
    ...
    private static TerminalPanelLine CloneLine(TerminalPanelLine line)
    {
        return new TerminalPanelLine(line.Text, line.Channel, line.Marker);
    }
