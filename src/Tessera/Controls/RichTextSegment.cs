...
public readonly record struct RichTextSegment
{
    ...
    {
        return new RichTextSegment(text);
    }
    ...
    public static RichTextSegment Heading(string? text, int level = 1)
    {
        ...
        return new RichTextSegment($"{new string('#', normalizedLevel)} {text ?? string.Empty}",
            RichTextStyleKind.Heading);
    }
    ...
    public static RichTextSegment ListMarker(string? marker = "-")
    {
        ...
        return new RichTextSegment($"{value} ", RichTextStyleKind.ListMarker);
    }
    ...
    {
        var value = string.IsNullOrWhiteSpace(marker) ? ">" : marker.Trim();
        return new RichTextSegment($"{value} ", RichTextStyleKind.QuoteMarker);
    }
    ...
    public static RichTextSegment Emphasis(string? text)
    {
        return new RichTextSegment(text, RichTextStyleKind.Emphasis);
    }
    ...
    public static RichTextSegment Strong(string? text)
    {
        return new RichTextSegment(text, RichTextStyleKind.Strong);
    }
    ...
    public static RichTextSegment InlineCode(string? text)
    {
        return new RichTextSegment(text, RichTextStyleKind.InlineCode);
    }
}
