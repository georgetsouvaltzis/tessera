...
public sealed class Wizard : Control
{
    ...
    public TesseraStyle StepStyle { get; set; } = TesseraStyle.Empty;
    ...
    public TesseraStyle FocusedActiveStepStyle { get; set; } = TesseraStyle.Empty;
    ...
    public TesseraStyle HoveredStepStyle { get; set; } = TesseraStyle.Empty;
    ...
    public WizardStep? CurrentStep => CurrentIndex >= 0 && CurrentIndex < _steps.Count ? _steps[CurrentIndex] : null;
    ...
    public void SetSteps(IEnumerable<WizardStep> steps)
    {
        ...
        CurrentIndex = ResolveInitialCurrentIndex(previousId);
        _hoveredIndex = -1;
        _scrollOffset = 0;
    ...
    public void SetSteps(IEnumerable<string> stepTitles)
    {
        ...
        SetSteps(stepTitles.Select(static value => new WizardStep(value, value)));
    }
    ...
    {
        return SetStepCompleted(CurrentIndex);
    }
    ...
    public override bool Handle(Message message)
    {
        ...
        if (key.Is(Key.Down) || key.Is(Key.Right) || key.IsCharacter('j'))
        {
            return NextStep();
        }

        if (key.Is(Key.Up) || key.Is(Key.Left) || key.IsCharacter('k'))
        {
            return PreviousStep();
        }
    ...
    public override bool Handle(Message message, Rect bounds)
    {
        ...
        if (pointer.Kind == PointerEventKind.Wheel)
        {
            ...
            {
                return NextStep() || changed;
            }

            if (pointer.Button == PointerButton.WheelUp)
            {
                return PreviousStep() || changed;
            }
        }
    ...
    internal override LayoutMeasurement Measure(in Rect availableBounds)
    {
        ...
        var height = Math.Max(1, _steps.Count) + (string.IsNullOrWhiteSpace(Title) ? 0 : 1) + Padding.Vertical +
    ...
    private void EnsureSelectionVisible(int viewportRows)
    {
        ...
        if (CurrentIndex < _scrollOffset)
        {
            _scrollOffset = CurrentIndex;
        }
        else if (CurrentIndex >= _scrollOffset + viewportRows)
        {
            _scrollOffset = CurrentIndex - viewportRows + 1;
        }
    ...
    {
        if (previousIndex == CurrentIndex && ReferenceEquals(previousStep, CurrentStep))
        {
            return;
        }

        var args = new WizardStepChangedEventArgs(previousIndex, CurrentIndex, previousStep, CurrentStep);
    ...
    private TesseraStyle ResolveStepStyle(int index)
    {
        ...
        else if (index == CurrentIndex)
        {
            ...
            if (IsFocused)
            {
                style = style.Merge(FocusedActiveStepStyle);
            }
        }
        ...
        {
            style = style.Merge(HoveredStepStyle);
        }
