...
public static class ThemeScope
{
    ...
    public static int Apply(TesseraTheme theme, params Control[] controls)
    {
    ...
    /// <returns>The number of controls that were successfully themed.</returns>
    public static int Apply(TesseraTheme theme, IEnumerable<Control?> controls)
    {
    ...
    private static Action<Control, TesseraTheme>? BuildApplyThemeDispatcher(Type controlType)
    {
        ...
        var theme = Expression.Parameter(typeof(TesseraTheme), "theme");
        var body = Expression.Call(
            applyTheme,
            Expression.Convert(control, extensionTarget),
            theme);
        return Expression.Lambda<Action<Control, TesseraTheme>>(body, control, theme).Compile();
    }
