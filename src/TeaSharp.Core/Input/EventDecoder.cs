...
public sealed class EventDecoder : IEventDecoder
{
    ...
    private static DecodeResult DecodeEscape(ReadOnlySpan<byte> buffer, bool timeoutExpired)
    {
        ...
        return buffer[1] switch
        {
            ...
            (byte)'P' => DecodeDcs(buffer, timeoutExpired),
    ...
    private static DecodeResult DecodeCsi(ReadOnlySpan<byte> buffer, bool timeoutExpired)
    {
        ...
        {
            var flags = TryParseInteger(buffer[3..finalIndex], out var parsedFlags)
                ? parsedFlags
                : 0;
    ...
    private static DecodeResult DecodeDcs(ReadOnlySpan<byte> buffer, bool timeoutExpired)
    {
    ...
    {
        var payload = ToAscii(payloadBytes);
        ...
        if (code == 10)
        {
            return new DecodeResult(consumed, new ForegroundColorMsg(NormalizeOscColor(data)), false);
        }

        if (code == 11)
        {
            return new DecodeResult(consumed, new BackgroundColorMsg(NormalizeOscColor(data)), false);
        }

        if (code == 12)
        {
            return new DecodeResult(consumed, new CursorColorMsg(NormalizeOscColor(data)), false);
        }
    ...
    private static bool TryParseInteger(ReadOnlySpan<byte> bytes, out int value)
    {
        value = 0;
        var seen = false;
