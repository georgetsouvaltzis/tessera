...
internal static class WidgetStatePaletteTests
{
    public static IEnumerable<TestCase> Cases()
    {
        ...
        yield return new TestCase("WidgetStatePalette_ChildOverridesParentStateAppearance",
            ChildOverridesParentStateAppearance);
    ...
    private static Task InheritsParentStateAppearance()
    {
        ...
        {
            TextStyle = TesseraStyle.Empty.WithForeground(AnsiColor.BrightBlue),
            Prefix = "P:"
        };

        var child = new WidgetStatePalette { Parent = parent };

        var rendered = child.Render("item", WidgetVisualState.Selected);

        TestAssert.True(rendered.Contains("P:item", StringComparison.Ordinal),
            "Child palette should inherit selected prefix from parent.");
        TestAssert.True(rendered.Contains("38;5;12", StringComparison.Ordinal),
            "Child palette should inherit selected color from parent.");
    ...
    private static Task ChildOverridesParentStateAppearance()
    {
        ...
        TestAssert.True(rendered.Contains("C:item", StringComparison.Ordinal),
            "Child palette should override selected prefix.");
        TestAssert.True(rendered.Contains("38;5;10", StringComparison.Ordinal),
            "Child palette should override selected color.");
        return Task.CompletedTask;
    }
    ...
    private static Task RejectsParentCycles()
    {
        ...
        var b = new WidgetStatePalette { Parent = a };
