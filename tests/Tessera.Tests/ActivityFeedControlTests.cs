...
public sealed class ActivityFeedControlTests
{
    ...
    {
        var control = new ActivityFeed { Border = BorderStyle.None, ShowTimestamp = false, AutoFollow = false };
        control.SetItems(
        [
            new ActivityFeedItem("alice", "deployed", "api", "green", ActivityFeedItemKind.Success,
                DateTimeOffset.UnixEpoch) { IsUnread = false },
            new ActivityFeedItem("bot", "alerted", "queue", "lag high", ActivityFeedItemKind.Warning,
                DateTimeOffset.UnixEpoch)
        ]);

        var output = Render(control, 96, 4);
    ...
    public void ControlsActivityFeedKeyboardAndPointerSelectionRaisesSelectionChanged()
    {
        ...
        var clickHandled = control.Handle(new PointerInput(PointerEventKind.Press, PointerButton.Left, 1, 2),
            new Rect(0, 0, 64, 4));
    ...
    public void ControlsActivityFeedStateStylesEmitAnsiAndDefaultRenderDeterministic()
    {
        ...
        styled.SetItems(
        [
            new ActivityFeedItem("svc", "ok", "api", kind: ActivityFeedItemKind.Success,
                timestamp: DateTimeOffset.UnixEpoch) { IsUnread = false },
            new ActivityFeedItem("svc", "failed", "worker", kind: ActivityFeedItemKind.Error,
                timestamp: DateTimeOffset.UnixEpoch)
        ]);
        _ = styled.Handle(new PointerInput(PointerEventKind.Motion, PointerButton.None, 1, 1), new Rect(0, 0, 96, 4));

        var styledOutput = Render(styled, 96, 4);
        ...
        var plain = new ActivityFeed { Border = BorderStyle.None, ShowTimestamp = false };
        plain.Append("bot", "noop", timestamp: DateTimeOffset.UnixEpoch);
        var first = Render(plain, 32, 2);
        var second = Render(plain, 32, 2);
