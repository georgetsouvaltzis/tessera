...
internal static partial class ThemeOverridesTests
{
    ...
    private static Task ApplyHelpers_MapExpectedTokens_ForWave1FormShellControls()
    {
        ...
        TestAssert.Equal(theme.State.Error, form.RequiredMarkerStyle,
            "Form required marker style should map to State.Error.");
        ...
        TestAssert.Equal(theme.Selection.Foreground.Merge(theme.Selection.Background), fieldSet.SelectedItemStyle,
            "FieldSet selected style should map to merged Selection styles.");
        TestAssert.Equal(theme.Border.Focused.Merge(theme.Focus.Border), fieldSet.FocusedBorderStyleText,
            "FieldSet focused border style should map to focused border tokens.");

        TestAssert.Equal(theme.Text.Muted, splitView.DividerStyle, "SplitView divider style should map to Text.Muted.");
        TestAssert.Equal(theme.Focus.Ring, splitView.FocusedDividerStyle,
            "SplitView focused divider style should map to Focus.Ring.");
        TestAssert.Equal(theme.Border.Default, splitView.BorderStyleText,
            "SplitView border style should map to Border.Default.");

        TestAssert.Equal(theme.Text.Secondary, inspector.SectionStyle,
            "InspectorPanel section style should map to Text.Secondary.");
        TestAssert.Equal(theme.Selection.Foreground.Merge(theme.Selection.Background), inspector.SelectedRowStyle,
            "InspectorPanel selected row style should map to merged Selection styles.");
        TestAssert.Equal(theme.Border.Focused.Merge(theme.Focus.Border), inspector.FocusedBorderStyleText,
            "InspectorPanel focused border style should map to focused border tokens.");

        TestAssert.Equal(theme.Accent.Primary, wizard.ActiveStepStyle,
            "Wizard active step style should map to Accent.Primary.");
        TestAssert.Equal(theme.State.Success, wizard.CompletedStepStyle,
            "Wizard completed step style should map to State.Success.");
        TestAssert.Equal(theme.Border.Default, wizard.BorderStyleText,
            "Wizard border style should map to Border.Default.");

        TestAssert.Equal(theme.Text.Secondary, dataForm.LabelStyle,
            "DataForm label style should map to Text.Secondary.");
        TestAssert.Equal(theme.Selection.Foreground.Merge(theme.Selection.Background), dataForm.SelectedFieldStyle,
            "DataForm selected field style should map to merged Selection styles.");
        TestAssert.Equal(theme.Border.Focused.Merge(theme.Focus.Border), dataForm.FocusedBorderStyleText,
            "DataForm focused border style should map to focused border tokens.");
    ...
    private static Task ApplyThemeDefaults_DoNotOverwriteExplicitStyles_ForWave1FormShellControls()
    {
        ...
        TestAssert.Equal(theme.State.Error, form.RequiredMarkerStyle,
            "Defaults should fill empty Form.RequiredMarkerStyle.");
        TestAssert.Equal(explicitStyle, form.BorderStyleText,
            "Defaults should not overwrite explicit Form.BorderStyleText.");

        TestAssert.Equal(explicitStyle, fieldSet.ItemStyle,
            "Defaults should not overwrite explicit FieldSet.ItemStyle.");
        TestAssert.Equal(theme.Accent.Secondary, fieldSet.HoveredItemStyle,
            "Defaults should fill empty FieldSet.HoveredItemStyle.");
        TestAssert.Equal(explicitStyle, fieldSet.BorderStyleText,
            "Defaults should not overwrite explicit FieldSet.BorderStyleText.");

        TestAssert.Equal(explicitStyle, splitView.DividerStyle,
            "Defaults should not overwrite explicit SplitView.DividerStyle.");
        TestAssert.Equal(theme.Focus.Ring, splitView.FocusedDividerStyle,
            "Defaults should fill empty SplitView.FocusedDividerStyle.");
        TestAssert.Equal(explicitStyle, splitView.BorderStyleText,
            "Defaults should not overwrite explicit SplitView.BorderStyleText.");

        TestAssert.Equal(explicitStyle, inspector.ValueStyle,
            "Defaults should not overwrite explicit InspectorPanel.ValueStyle.");
        TestAssert.Equal(theme.Accent.Secondary, inspector.MarkerStyle,
            "Defaults should fill empty InspectorPanel.MarkerStyle.");
        TestAssert.Equal(explicitStyle, inspector.BorderStyleText,
            "Defaults should not overwrite explicit InspectorPanel.BorderStyleText.");

        TestAssert.Equal(explicitStyle, wizard.StepStyle, "Defaults should not overwrite explicit Wizard.StepStyle.");
        TestAssert.Equal(theme.State.Success, wizard.CompletedStepStyle,
            "Defaults should fill empty Wizard.CompletedStepStyle.");
        TestAssert.Equal(explicitStyle, wizard.BorderStyleText,
            "Defaults should not overwrite explicit Wizard.BorderStyleText.");

        TestAssert.Equal(explicitStyle, dataForm.ValueStyle,
            "Defaults should not overwrite explicit DataForm.ValueStyle.");
        TestAssert.Equal(theme.Text.Muted, dataForm.PlaceholderStyle,
            "Defaults should fill empty DataForm.PlaceholderStyle.");
        TestAssert.Equal(explicitStyle, dataForm.BorderStyleText,
            "Defaults should not overwrite explicit DataForm.BorderStyleText.");
    ...
    private static Task OverrideOverloads_ResolveExpectedTokens_ForWave1FormShellControls()
    {
        ...
        TestAssert.Equal(explicitStyle, form.ValueStyle,
            "Override defaults should not overwrite explicit Form.ValueStyle.");
        TestAssert.Equal(typeTheme.Border.Default, form.BorderStyleText,
            "Override defaults should fill empty Form.BorderStyleText.");

        TestAssert.Equal(explicitStyle, fieldSet.ItemStyle,
            "Override defaults should not overwrite explicit FieldSet.ItemStyle.");
        TestAssert.Equal(typeTheme.Selection.Foreground.Merge(typeTheme.Selection.Background),
            fieldSet.SelectedItemStyle, "Override defaults should fill empty FieldSet.SelectedItemStyle.");

        TestAssert.Equal(explicitStyle, splitView.DividerStyle,
            "Override defaults should not overwrite explicit SplitView.DividerStyle.");
        TestAssert.Equal(typeTheme.Border.Default, splitView.BorderStyleText,
            "Override defaults should fill empty SplitView.BorderStyleText.");

        TestAssert.Equal(explicitStyle, inspector.ValueStyle,
            "Override defaults should not overwrite explicit InspectorPanel.ValueStyle.");
        TestAssert.Equal(typeTheme.Accent.Secondary, inspector.MarkerStyle,
            "Override defaults should fill empty InspectorPanel.MarkerStyle.");

        TestAssert.Equal(explicitStyle, wizard.StepStyle,
            "Override defaults should not overwrite explicit Wizard.StepStyle.");
        TestAssert.Equal(typeTheme.State.Success, wizard.CompletedStepStyle,
            "Override defaults should fill empty Wizard.CompletedStepStyle.");

        TestAssert.Equal(explicitStyle, dataForm.ValueStyle,
            "Override defaults should not overwrite explicit DataForm.ValueStyle.");
        TestAssert.Equal(typeTheme.Text.Muted, dataForm.PlaceholderStyle,
            "Override defaults should fill empty DataForm.PlaceholderStyle.");
        TestAssert.Equal(typeTheme.Border.Default, dataForm.BorderStyleText,
            "Override defaults should fill empty DataForm.BorderStyleText.");
