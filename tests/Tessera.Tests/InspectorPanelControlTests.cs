...
public sealed class InspectorPanelControlTests
{
    ...
    {
        var general = new InspectorSection("General");
        general.AddField("Name", "Tessera");
        general.AddDetail("interactive mode");
        var metrics = new InspectorSection("Metrics", false);
        metrics.AddField("Rows", "24");

        var control = new InspectorPanel { Border = BorderStyle.None };
        ...
        TestAssert.True(output.Contains("▾ General", StringComparison.Ordinal),
            "Expanded section should render expanded marker.");
        ...
        TestAssert.True(output.Contains("▸ Metrics", StringComparison.Ordinal),
            "Collapsed section should render collapsed marker.");
    }

    [Test]
    public void ControlsInspectorPanelKeyboardNavigationAndToggleSection()
    {
        var section = new InspectorSection("Build", false);
        section.AddField("Target", "Release");
        var control = new InspectorPanel { Border = BorderStyle.None, IsFocused = true };
        ...
        TestAssert.True(!output.Contains("Target", StringComparison.Ordinal),
            "Collapsed section should hide field rows.");
    }

    [Test]
    public void ControlsInspectorPanelPointerSelectionAndToggleSection()
    {
        var section = new InspectorSection("Env", false);
        section.AddField("Region", "eu-west");
        var control = new InspectorPanel { Border = BorderStyle.None };
        control.SetSections([section]);
        var bounds = new Rect(0, 0, 40, 6);

        var headerPress = control.Handle(new PointerInput(PointerEventKind.Press, PointerButton.Left, 1, 0), bounds);
        var rowPress = control.Handle(new PointerInput(PointerEventKind.Press, PointerButton.Left, 2, 1), bounds);
    ...
    {
        var section = new InspectorSection("Runtime");
        ...
        TestAssert.True(output.Contains("48;2;7;17;27", StringComparison.Ordinal),
            "Selected row style should emit ANSI background sequence.");
    }

    [Test]
    public void ControlsInspectorPanelDefaultRenderIsDeterministicAndMonochrome()
    {
        var section = new InspectorSection("Config");
        ...
        TestAssert.True(!first.Contains("\e[", StringComparison.Ordinal),
            "Default InspectorPanel output should remain monochrome.");
    }

    private static string Render(InspectorPanel control, int width, int height,
