...
internal static partial class ThemeOverridesTests
{
    ...
    private static Task ApplyHelpers_MapExpectedTokens_ForWave4WorkspaceControls()
    {
        ...
        TestAssert.Equal(theme.Border.Default, workspace.BorderStyleText,
            "DockWorkspace border style should map to Border.Default.");
        TestAssert.Equal(theme.Selection.Foreground.Merge(theme.Selection.Background), workspace.SelectedPaneBodyStyle,
            "DockWorkspace selected pane body style should map to merged Selection styles.");

        TestAssert.Equal(theme.Text.Primary, tabs.TabStyle, "PaneTabs tab style should map to Text.Primary.");
        TestAssert.Equal(theme.Border.Focused.Merge(theme.Focus.Border), tabs.FocusedBorderStyleText,
            "PaneTabs focused border style should map to focused border tokens.");

        TestAssert.Equal(theme.State.Success, heatmap.PeakCellStyle,
            "Heatmap peak cell style should map to State.Success.");
        TestAssert.Equal(theme.Selection.Foreground.Merge(theme.Selection.Background), heatmap.SelectedCellStyle,
            "Heatmap selected cell style should map to merged Selection styles.");

        TestAssert.Equal(theme.Text.Secondary, processList.HeaderStyle,
            "ProcessListView header style should map to Text.Secondary.");
        TestAssert.Equal(theme.Border.Default, processList.BorderStyleText,
            "ProcessListView border style should map to Border.Default.");

        TestAssert.Equal(theme.Text.Primary, treeMap.NodeStyle, "TreeMapChart node style should map to Text.Primary.");
        TestAssert.Equal(theme.Border.Focused.Merge(theme.Focus.Border), treeMap.FocusedBorderStyleText,
            "TreeMapChart focused border style should map to focused border tokens.");

        TestAssert.Equal(theme.Text.Primary, paletteEditor.SwatchStyle,
            "PaletteEditor swatch style should map to Text.Primary.");
        TestAssert.Equal(theme.Focus.Ring, paletteEditor.FocusedSelectedSwatchStyle,
            "PaletteEditor focused selected style should map to Focus.Ring.");

        TestAssert.Equal(theme.State.Error, terminalPanel.StandardErrorStyle,
            "TerminalPanel stderr style should map to State.Error.");
        TestAssert.Equal(theme.Selection.Foreground.Merge(theme.Selection.Background), terminalPanel.SelectedLineStyle,
            "TerminalPanel selected style should map to merged Selection styles.");
    ...
    private static Task ApplyThemeDefaults_DoNotOverwriteExplicitStyles_ForWave4WorkspaceControls()
    {
        ...
        var theme = new TesseraTheme
        {
            ...
            State = new TesseraThemeStateTokens
            {
                Success = TesseraStyle.Empty.WithForeground(AnsiColor.Rgb(38, 39, 40)),
        ...
        TestAssert.Equal(explicitStyle, workspace.PaneBodyStyle,
            "Defaults should not overwrite explicit DockWorkspace.PaneBodyStyle.");
        TestAssert.Equal(theme.Text.Secondary, workspace.PaneTitleStyle,
            "Defaults should fill empty DockWorkspace.PaneTitleStyle.");
        TestAssert.Equal(explicitStyle, workspace.BorderStyleText,
            "Defaults should not overwrite explicit DockWorkspace.BorderStyleText.");

        TestAssert.Equal(explicitStyle, tabs.TabStyle, "Defaults should not overwrite explicit PaneTabs.TabStyle.");
        TestAssert.Equal(theme.Accent.Secondary, tabs.HoveredTabStyle,
            "Defaults should fill empty PaneTabs.HoveredTabStyle.");
        TestAssert.Equal(explicitStyle, tabs.BorderStyleText,
            "Defaults should not overwrite explicit PaneTabs.BorderStyleText.");

        TestAssert.Equal(explicitStyle, heatmap.CellStyle, "Defaults should not overwrite explicit Heatmap.CellStyle.");
        TestAssert.Equal(theme.State.Success, heatmap.PeakCellStyle,
            "Defaults should fill empty Heatmap.PeakCellStyle.");
        TestAssert.Equal(explicitStyle, heatmap.BorderStyleText,
            "Defaults should not overwrite explicit Heatmap.BorderStyleText.");

        TestAssert.Equal(explicitStyle, processList.RowStyle,
            "Defaults should not overwrite explicit ProcessListView.RowStyle.");
        TestAssert.Equal(theme.Text.Secondary, processList.HeaderStyle,
            "Defaults should fill empty ProcessListView.HeaderStyle.");
        TestAssert.Equal(explicitStyle, processList.BorderStyleText,
            "Defaults should not overwrite explicit ProcessListView.BorderStyleText.");

        TestAssert.Equal(explicitStyle, treeMap.NodeStyle,
            "Defaults should not overwrite explicit TreeMapChart.NodeStyle.");
        TestAssert.Equal(theme.Accent.Primary, treeMap.HighNodeStyle,
            "Defaults should fill empty TreeMapChart.HighNodeStyle.");
        TestAssert.Equal(explicitStyle, treeMap.BorderStyleText,
            "Defaults should not overwrite explicit TreeMapChart.BorderStyleText.");

        TestAssert.Equal(explicitStyle, paletteEditor.SwatchStyle,
            "Defaults should not overwrite explicit PaletteEditor.SwatchStyle.");
        TestAssert.Equal(theme.Accent.Primary, paletteEditor.PreviewSwatchStyle,
            "Defaults should fill empty PaletteEditor.PreviewSwatchStyle.");

        TestAssert.Equal(explicitStyle, terminalPanel.StandardOutputStyle,
            "Defaults should not overwrite explicit TerminalPanel.StandardOutputStyle.");
        TestAssert.Equal(theme.State.Error, terminalPanel.StandardErrorStyle,
            "Defaults should fill empty TerminalPanel.StandardErrorStyle.");
    ...
    private static Task OverrideOverloads_ResolveExpectedTokens_ForWave4WorkspaceControls()
    {
        ...
        var typeTheme = new TesseraTheme
        {
            ...
            State = new TesseraThemeStateTokens
            {
                Success = TesseraStyle.Empty.WithForeground(AnsiColor.Rgb(181, 182, 183)),
        ...
        TestAssert.Equal(explicitStyle, workspace.PaneBodyStyle,
            "Override defaults should not overwrite explicit DockWorkspace.PaneBodyStyle.");
        TestAssert.Equal(typeTheme.Border.Default, workspace.BorderStyleText,
            "Override defaults should fill empty DockWorkspace.BorderStyleText.");

        TestAssert.Equal(explicitStyle, tabs.TabStyle,
            "Override defaults should not overwrite explicit PaneTabs.TabStyle.");
        TestAssert.Equal(typeTheme.Selection.Foreground.Merge(typeTheme.Selection.Background), tabs.SelectedTabStyle,
            "Override defaults should fill empty PaneTabs.SelectedTabStyle.");

        TestAssert.Equal(explicitStyle, heatmap.CellStyle,
            "Override defaults should not overwrite explicit Heatmap.CellStyle.");
        TestAssert.Equal(typeTheme.State.Success, heatmap.PeakCellStyle,
            "Override defaults should fill empty Heatmap.PeakCellStyle.");

        TestAssert.Equal(explicitStyle, processList.RowStyle,
            "Override defaults should not overwrite explicit ProcessListView.RowStyle.");
        TestAssert.Equal(typeTheme.Border.Default, processList.BorderStyleText,
            "Override defaults should fill empty ProcessListView.BorderStyleText.");

        TestAssert.Equal(explicitStyle, treeMap.NodeStyle,
            "Override defaults should not overwrite explicit TreeMapChart.NodeStyle.");
        TestAssert.Equal(typeTheme.Accent.Primary, treeMap.HighNodeStyle,
            "Override defaults should fill empty TreeMapChart.HighNodeStyle.");

        TestAssert.Equal(explicitStyle, paletteEditor.SwatchStyle,
            "Override defaults should not overwrite explicit PaletteEditor.SwatchStyle.");
        TestAssert.Equal(typeTheme.Accent.Primary, paletteEditor.PreviewSwatchStyle,
            "Override defaults should fill empty PaletteEditor.PreviewSwatchStyle.");

        TestAssert.Equal(explicitStyle, terminalPanel.StandardOutputStyle,
            "Override defaults should not overwrite explicit TerminalPanel.StandardOutputStyle.");
        TestAssert.Equal(typeTheme.State.Error, terminalPanel.StandardErrorStyle,
            "Override defaults should fill empty TerminalPanel.StandardErrorStyle.");
