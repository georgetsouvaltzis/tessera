...
internal static class PublicOnboardingExamplesTests
{
    ...
    public static IEnumerable<TestCase> Cases()
    {
        yield return new TestCase(
            ...
            Projects_Exist);
        yield return new TestCase(
            "PublicOnboardingExamples_ProgramsDoNotImportTesseraCore",
            Programs_DoNotImportTesseraCore);
        yield return new TestCase(
            "PublicOnboardingExamples_ProgramsDoNotImportDependencyInjection",
            Programs_DoNotImportDependencyInjection);
        yield return new TestCase(
            "PublicOnboardingExamples_ProgramsDoNotImportTesseraHosting",
            Programs_DoNotImportTesseraHosting);
        yield return new TestCase(
            "PublicOnboardingExamples_ProjectsDoNotReferenceTesseraCoreProject",
            Projects_DoNotReferenceTesseraCoreProject);
    }
    ...
    private static Task Programs_DoNotImportTesseraCore()
    {
        AssertProgramsDoNotMatch(
            TesseraCoreImportRegex,
            "Onboarding examples must not import Tessera.Core.*.");
        return Task.CompletedTask;
    }
    ...
    private static Task Programs_DoNotImportTesseraHosting()
    {
        AssertProgramsDoNotMatch(
            TesseraHostingImportRegex,
            "Onboarding examples should not import Tessera.Hosting in the default public path.");
        return Task.CompletedTask;
    }

    private static Task Projects_DoNotReferenceTesseraCoreProject()
    {
        var repoRoot = GetRepoRoot();
        ...
        TestAssert.True(
            offenders.Length == 0,
            $"Onboarding example projects must not reference Tessera.Core.csproj directly. Offenders: {string.Join(", ", offenders)}.");
