...
public sealed class NUnitCaseAdapters
{
    public static IEnumerable<TestCaseData> Cases()
    {
        ...
        foreach (var testCase in ThemeRuntimeIntegrationTests.Cases()) yield return ToCaseData(testCase);
        foreach (var testCase in TesseraControlCatalogTests.Cases()) yield return ToCaseData(testCase);
        foreach (var testCase in TesseraAppCompositionTests.Cases()) yield return ToCaseData(testCase);
        foreach (var testCase in TesseraAppFoundationTests.Cases()) yield return ToCaseData(testCase);
        foreach (var testCase in TesseraApplicationBuilderContractTests.Cases()) yield return ToCaseData(testCase);
        ...
        foreach (var testCase in InteractionPointerParity_NavigationExplorerTests.Cases()) yield return ToCaseData(testCase);
        foreach (var testCase in ThemeStateParity_NavigationExplorerTests.Cases()) yield return ToCaseData(testCase);
