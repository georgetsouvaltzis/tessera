...
public sealed class QueryBuilderControlTests
{
    ...
    public void Controls_QueryBuilder_StateStyles_RenderExpectedAnsi()
    {
        var control = new QueryBuilder
        {
            ...
            IsFocused = true,
            HasError = true,
            SelectedRuleStyle = TeaStyle.Empty.WithBackground(AnsiColor.Rgb(1, 2, 3)),
            FocusedRuleStyle = TeaStyle.Empty.WithItalic(),
            HoveredRuleStyle = TeaStyle.Empty.WithUnderline(),
            DisabledRuleStyle = TeaStyle.Empty.WithDim(),
            ErrorRuleStyle = TeaStyle.Empty.WithForeground(AnsiColor.Rgb(31, 32, 33)),
        };
        control.SetRules(
        [
            new QueryRule("status", QueryOperator.Equals, "open"),
