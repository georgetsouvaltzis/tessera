...
public sealed class AreaPlotControlTests
{
    ...
    {
        var focusedTitle = TeaStyle.Empty.WithUnderline().WithForeground(AnsiColor.BrightCyan);
        var control = new AreaPlot
        {
            Border = BorderStyle.SingleLine,
            Title = "Memory",
    ...
    public void AreaPlotDisabledStyleIsMergedIntoFillAndLineStyles()
    {
        var control = new AreaPlot
        {
            ...
            MinValue = 0,
            MaxValue = 100,
            FillStyle = TeaStyle.Empty.WithForeground(AnsiColor.BrightGreen),
            LineStyle = TeaStyle.Empty.WithForeground(AnsiColor.BrightBlue),
