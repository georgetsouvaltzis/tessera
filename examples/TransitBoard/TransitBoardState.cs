...
internal sealed class TransitBoardState
{
    ...
    public string ClockText =>
        DateTimeOffset.UtcNow.AddSeconds(_tick).ToString("HH:mm:ss", CultureInfo.InvariantCulture);
    ...
    public static TransitBoardState CreateSeed()
    {
        ...
        [
            new("all", "All", "network"),
            new("aero", "Aero", "airport"),
            new("coast", "Coast", "harbor"),
            new("civic", "Civic", "loop"),
            new("inter", "Inter", "fast"),
            new("night", "Night", "pier")
        ];

        List<TransitService> services =
        [
            Service("svc_170", "A1", "AeroLink", "North Terminal", "via Skybridge", "4", now.AddMinutes(3), false, 0,
                "boarding", "All doors open", "Gate 4 stable", "Concourse B",
                Calls("Central Exchange", "Museum Dock", "Skybridge", "North Terminal", now.AddMinutes(3))),
            Service("svc_182", "C7", "Coastline", "Harbor Point", "via Customs Hall", "7", now.AddMinutes(5), false, 4,
                "delayed", "+04 weather check", "Gate 7 -> 9", "Concourse A",
                Calls("Central Exchange", "Canal Yard", "Customs Hall", "Harbor Point", now.AddMinutes(5))),
            Service("svc_191", "L2", "Civic Loop", "Old Market", "via River Walk", "2", now.AddMinutes(7), false, 0,
                "final call", "Doors closing", "Gate 2 stable", "Concourse C",
                Calls("Central Exchange", "River Walk", "Civic Square", "Old Market", now.AddMinutes(7))),
            Service("svc_205", "I4", "InterCity", "Northgate", "via Meridian", "11", now.AddMinutes(12), false, 0,
                "on time", "Express formation", "Gate 11 stable", "Platform Hall",
                Calls("Central Exchange", "Meridian", "West Spur", "Northgate", now.AddMinutes(12))),
            Service("svc_218", "N8", "Night Ferry", "Dock Eight", "via Glass Pier", "15", now.AddMinutes(15), false, 2,
                "gate change", "+02 boarding hold", "Gate 15 -> 12", "Pier Floor",
                Calls("Central Exchange", "Glass Pier", "Low Water", "Dock Eight", now.AddMinutes(15))),
            Service("svc_231", "A3", "AeroLink", "South Terminal", "via Runway South", "5", now.AddMinutes(18), false,
                0, "on time", "Heavy luggage flow", "Gate 5 stable", "Concourse B",
                Calls("Central Exchange", "Runway South", "Cargo Annex", "South Terminal", now.AddMinutes(18))),
            Service("svc_246", "C1", "Coastline", "Lighthouse", "via Salt Yard", "8", now.AddMinutes(22), false, 0,
                "on time", "Rain sweep active", "Gate 8 stable", "Concourse A",
                Calls("Central Exchange", "Salt Yard", "Marina South", "Lighthouse", now.AddMinutes(22))),
            Service("svc_259", "I2", "InterCity", "Stonebridge", "via Northgate", "10", now.AddMinutes(27), false, 9,
                "delayed", "+09 upstream hold", "Gate 10 stable", "Platform Hall",
                Calls("Central Exchange", "Northgate", "Red Vale", "Stonebridge", now.AddMinutes(27))),
            Service("svc_301", "A1", "AeroLink", "Central Exchange", "from North Terminal", "4", now.AddMinutes(2),
                true, 0, "landing", "Arriving platform 4", "Gate 4 stable", "Concourse B",
                Calls("North Terminal", "Skybridge", "Museum Dock", "Central Exchange", now.AddMinutes(2))),
            Service("svc_309", "C7", "Coastline", "Central Exchange", "from Harbor Point", "9", now.AddMinutes(6), true,
                3, "approaching", "+03 tidal buffer", "Gate 9 stable", "Concourse A",
                Calls("Harbor Point", "Customs Hall", "Canal Yard", "Central Exchange", now.AddMinutes(6))),
            Service("svc_316", "L2", "Civic Loop", "Central Exchange", "from Old Market", "2", now.AddMinutes(9), true,
                0, "arriving", "Loop frequency restored", "Gate 2 stable", "Concourse C",
                Calls("Old Market", "Civic Square", "River Walk", "Central Exchange", now.AddMinutes(9))),
            Service("svc_324", "I4", "InterCity", "Central Exchange", "from Northgate", "11", now.AddMinutes(14), true,
                0, "arriving", "Fast consist", "Gate 11 stable", "Platform Hall",
                Calls("Northgate", "West Spur", "Meridian", "Central Exchange", now.AddMinutes(14)))
        ];

        List<TransitNotice> notices =
        [
            new("amber", "Platform 9", "Harbor point boarding moved from 7 to 9 due to wind corridor."),
            new("blue", "Skybridge", "Airport baggage screening is adding two minutes to AeroLink dwell time."),
            new("mint", "Civic Loop", "Inner-city headways normalized after crowd release from museum district."),
            new("rose", "Dock Eight", "Night Ferry footbridge under one-way movement after maintenance sweep.")
        ];
    ...
    {
        return _services.FirstOrDefault(service => string.Equals(service.Id, id, StringComparison.Ordinal));
    }
    ...
    public static string BuildAdvisory(IReadOnlyList<TransitService> visible)
    {
        ...
        return
            $"Next movement: {next.RouteCode} to {next.Destination} from platform {next.Platform}  ·  {marker}  ·  {next.Concourse}";
    }
    ...
    {
        return new TransitService(id, routeLabel.ToLowerInvariant()[..Math.Min(5, routeLabel.Length)], routeCode,
            routeLabel, destination, via, platform, departsAt, isArrival, delayMinutes, status, markerText, gateChange,
            concourse, calls);
    }

    private static List<TransitCall> Calls(string first, string second, string third, string fourth,
    ...
    private static TransitChipItem ModeItem(string id, string label, string subtitle, bool selected,
        TransitBoardPalette palette)
    {
    ...
    private static TransitChipItem ThemeItem(TransitBoardThemeKind kind, TransitBoardThemeKind selectedTheme,
        TransitBoardPalette palette)
    {
...
internal sealed record TransitChipItem(
    string Id,
    string Label,
    string Subtitle,
    TesseraStyle PrimaryStyle,
    TesseraStyle SecondaryStyle);
